...
- **Reconocimiento preciso con Whisper (opcional)**: Apple sigue detectando "hey Claude" y mostrando la orden en vivo; al terminar, whisper.cpp (local, con Metal) transcribe el audio de la orden usando tu vocabulario y correcciones como pista, así nombres como "Hikaru" o "chess.com" salen bien. Se instala con `tts/setup_whisper.sh` (o el botón en Ajustes → Conversación) y añade entre medio segundo y un segundo al final de cada orden; si falla o tarda más de 2.5 s se usa el texto de Apple.
...
- **Mensajes y notificaciones**: "¿qué notificaciones tengo?" lee el Centro de notificaciones; "léeme el último mensaje" o "los últimos 3 mensajes de Ana" leen Mensajes/iMessage (requiere Acceso total al disco para Claude Voice); "léeme mis mensajes de WhatsApp", "los últimos 3 mensajes de WhatsApp de Ana" o "¿tengo mensajes nuevos en WhatsApp?" leen la base local de la app de WhatsApp, al instante y sin permisos extra. Sin modelo, al instante.
...
- **"¿Qué puedes hacer?"**: responde con ideas para ahora mismo y abre un panel de tarjetas animadas sobre el widget: las naranjas salen de tu contexto, tu uso, la hora y lo que tienes abierto; el resto, del repertorio. Toca una para probarla (las que terminan en "…" te dejan completarla por voz).
