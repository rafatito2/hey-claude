...
| `tts/setup_whisper.sh` | Instala `whisper-cpp` por Homebrew y descarga el modelo a `~/claude-voice/whisper/` (large-v3-turbo q5_0 por defecto, small opcional). La app arranca `whisper-server` en 127.0.0.1:8790 (`WhisperASR`) y lo apaga al cerrar. |
...
- **Whisper híbrido** (`WhisperASR`, ajuste `whisperASR`): el reconocedor de Apple se conserva para la activación y los parciales; el `Listener` guarda los últimos 40 s de audio 16 kHz en un búfer circular y, al terminar una orden de voz de 3 palabras o más, `commit` manda el audio desde `listenStart` a `whisper-server` (/inference, multipart, `language` y `prompt` con vocabulario + correcciones, tope 2.5 s) y `commitFinal` envía el texto afinado; con fallo o tardanza se envía el de Apple. Las órdenes escritas y las rutinas no se afinan; `refining` bloquea el tick de escucha mientras tanto.
...
- **Sugerencias** (`SuggestionsPanel` en tasks.swift, `buildSuggestions` en el controlador): ruta local "¿qué puedes hacer?"; hasta 3 personalizadas (tarea en curso, silencio, hora, `contexto.md`, apps abiertas, orden más repetida en voice.log, sin rutinas) + generales barajadas con semilla día+hora; tarjetas que entran escalonadas (alpha + CABasicAnimation de translation.y con sobreimpulso), borde naranja las personales; tocar una ejecuta `runSuggestion` (con "…" abre la escucha con el texto de semilla). Se oculta al llegar otra orden o a los 45 s.
- **Terminal en el panel**: `PersistentClaude.terminal` guarda los `tool_use` de Bash (comando) y los empareja con los `tool_result` que llegan en mensajes `user` (salida, `is_error`); `TasksPanel.terminalBox` los pinta (fondo oscuro, `$` verde, salida gris o roja, "ejecutando…" mientras no hay salida). El panel se muestra también sin tareas mientras la conversación ejecuta comandos y hasta 20 s después.
